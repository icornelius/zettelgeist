...
def get_argparse():
    ...
    parser.add_argument(
        "--publish",
        help="use template to publish zfind output (suppresses all --show-FIELD)",
        default=None,
    )

    parser.add_argument(
        "--publish-conf",
        help="use publish configuration file to format special fields",
        default=None,
    )
    ...
    parser.add_argument(
        "--query-file",
        help="load ZQL query from file (overrides --query)",
        default=None,
    )

    parser.add_argument("--query-string", help="load ZQL from string", default=None)

    parser.add_argument("--fileset", help="write fileset to filename", default=None)
    ...
    parser.add_argument(
        "--get-all-tags",
        help="show all tags in this database (disables all other options)",
        action="store_true",
        default=False,
    )
    ...
    parser.add_argument(
        "--get-all-mentions",
        help="show all mentions in this database (disables all other options)",
        action="store_true",
        default=False,
    )
...
def main():
    ...
    if input_line:
        ...
        print(
            "Warning: Query could not be loaded via --query, --query-file, or --query-prompt"
        )
        sys.exit(1)
...
def reformat_tags(all_vars, formatting):
    ...
    return (
        tags_format.get("before", "")
        + "".join(formatted_tags)
        + tags_format.get("after", "")
    )
...
def reformat_mentions(all_vars, formatting):
    ...
    formatted_mentions = [
        mentions_format.get("mention", default_mention_format) % {"mention": mention}
        for mention in mentions
    ]
    return (
        mentions_format.get("before", "")
        + "".join(formatted_mentions)
        + mentions_format.get("after", "")
    )
